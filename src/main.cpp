...
pcl::PointCloud<pcl::PointXYZ> unlabeled_pc;
...
{
    // ROS Stuff
    ros::init(argc, argv, "natnet_ros_node");
    ros::NodeHandle nh;
    ...
    printf( "NatNet ROS Client (NatNet ver. %d.%d.%d.%d)\n", ver[0], ver[1], ver[2], ver[3] );
    ...
    if ( argc == 1 )
    {
        ...
        printf("Marker [ModelID, MarkerID, Occluded, PCSolved, ModelSolved] [size] [pos (x, y, z)]\n");
...
//    }
    ...
    for(i=0; i < data->nLabeledMarkers; i++)
    {
        ...
        auto str_msg =
                    std::to_string(data->fTimestamp) + "," +
                    std::to_string(data->iFrame) + "," +
                    std::to_string(modelID) + "," +
                    std::to_string(markerID) + "," +
                    std::to_string(bOccluded) + "," +
                    std::to_string(bPCSolved) + "," +
                    std::to_string(bModelSolved) + "," +
                    // Marker Data
                    std::to_string(marker.size) + "," +
                    std::to_string(marker.x) + "," +
                    std::to_string(marker.y) + "," +
                    std::to_string(marker.z);

        std_msgs::String str;
        str.data = str_msg.c_str();
        raw_pub.publish(str);
        if (szMarkerType == "Unlabeled"){
            //
            unlabeled_pc.points.emplace_back(marker.x, marker.y, marker.z);
            pcl::toROSMsg(unlabeled_pc, pc_msg);
            //
        }
